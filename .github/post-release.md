...
4. Publishing uses npm [trusted publishing](https://docs.npmjs.com/trusted-publishers) (OIDC) only — there is no `NPM_TOKEN` fallback. The release workflow sets `id-token: write`. Before the first non-dry run, configure a Trusted Publisher on npmjs.com for `@elastic/distillate`: GitHub Actions, repository `elastic/distillate`, workflow `release.yml`. Provenance is emitted automatically under trusted publishing. The supported consumer install is `npm install @elastic/distillate` from registry.npmjs.org.
