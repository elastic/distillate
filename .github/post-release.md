...
4. Publishing needs an `NPM_TOKEN` repository secret for `@elastic/distillate`, plus a `GITHUB_TOKEN` that can create releases and push the version bump. Trusted publishing / OIDC is not configured for this workflow. The release workflow exports the npm token as both `NPM_TOKEN` for `@semantic-release/npm` and `NODE_AUTH_TOKEN` for the npm registry auth written by `actions/setup-node`. Configure `NPM_TOKEN` before a non-dry run. The supported consumer install is `npm install @elastic/distillate` from registry.npmjs.org, the same contract as `@elastic/eui`.
