...
4. First run it with **dry_run** enabled. That exercises `pnpm verify` and semantic-release without publishing. With no prior git tag, the first `main` release is **0.1.0** (`scripts/run_semantic_release.js` patches semantic-release's hardcoded `1.0.0`). Confirm the log says `the next release version is 0.1.0`, not `1.0.0`.
