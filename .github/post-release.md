...
4. Publishing needs the package on [npm trusted publishing](https://docs.npmjs.com/trusted-publishers) for `@elastic/distillate` (OIDC) or an `NPM_TOKEN` repository secret, plus a `GITHUB_TOKEN` that can create releases and push the version bump. Trusted publishing needs npm 11.5.1 or later; the release workflow installs npm 11 on Node 22 so OIDC works. `id-token: write` is already set. Configure trusted publishing or `NPM_TOKEN` before a non-dry run. The supported consumer install is `npm install @elastic/distillate` from registry.npmjs.org, the same contract as `@elastic/eui`.
