...
5. A successful non-dry run tags the version, publishes `@elastic/distillate` to the npm registry, and creates a GitHub Release. It does not push commits to `main` (Elastic requires a pull request). Notes live on the GitHub Release; `package.json` on `main` is not the published version. Later `feat:` commits become `0.2.0`, `0.3.0`, …; a `BREAKING CHANGE` becomes `1.0.0`. The supported consumer install is `npm install @elastic/distillate` from registry.npmjs.org.
