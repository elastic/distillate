...
`themeValueOverrides` still apply per render for values known only at request time. Declared variations are for values known when the distillery is created. A non-CSS consumer that needs the same snapshot calls `distillery.resolveValues(scheme, variation?)`; see [read values outside CSS](non-css-surfaces.md).
