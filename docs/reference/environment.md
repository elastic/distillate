...
`Distillery.tokens` and `Distillery.themeVars` alias the same objects. `Distillery.resolveValues(scheme, variation?)` reshapes them into a nested literal tree for one scheme. See [read values outside CSS](../guides/non-css-surfaces.md).
